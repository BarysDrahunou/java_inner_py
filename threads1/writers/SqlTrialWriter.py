...
TRIAL_SQL_SERIALIZERS_DICT = {TRIAL: TrialSqlSerializer(),
...
class SqlTrialWriter:
    ...
    def __init__(self, *args):
        ...
        if not is_table_exist(database_name, table_name):
            connection = get_connection(database_name, configuration_file_name)
            cursor = connection.cursor()
            if not is_database_exist(database_name, configuration_file_name):
    ...
    def write_trial(self, trial):
        ...
        values = TRIAL_SQL_SERIALIZERS_DICT.get(camel_to_snake(trial.__class__.__name__)) \
            .serialize(trial)
